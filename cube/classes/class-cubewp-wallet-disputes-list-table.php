<?php
/**
 * CubeWP Wallet Disputes List Table.
 *
...
/**
 * CubeWp_Wallet_Disputes_List_Table
 */
class CubeWp_Wallet_Disputes_List_Table extends WP_List_Table {
	...
	function column_ID( $item ) {
		...
		$actions = array(
			'cubewp-view-dispute-details' => sprintf( '<a href="#" data-target-id="%s">' . esc_html__( 'View Details', 'cubewp-wallet' ) . '</a>', $item['ID'] )
		);
		if ( $status == 'pending' ) {
			$actions['approve_dispute'] = sprintf( '<a href="%s">' . esc_html__( 'Approve', 'cubewp-wallet' ) . '</a>', CubeWp_Submenu::_page_action( 'cubewp-wallet-disputes', 'approve', '&item_id=' . $item['ID'], '&nonce=' . wp_create_nonce( 'cubewp_wallet_approve_record_nonce' ) ) );
			$actions['trash'] = sprintf( '<a href="%s">' . esc_html__( 'Reject', 'cubewp-wallet' ) . '</a>', CubeWp_Submenu::_page_action( 'cubewp-wallet-disputes', 'reject', '&item_id=' . $item['ID'], '&nonce=' . wp_create_nonce( 'cubewp_wallet_reject_record_nonce' ) ) );
		}else {
			$actions['delete'] = sprintf( '<a href="%s">' . esc_html__( 'Delete', 'cubewp-wallet' ) . '</a>', CubeWp_Submenu::_page_action( 'cubewp-wallet-disputes', 'delete', '&item_id=' . $item['ID'], '&nonce=' . wp_create_nonce( 'cubewp_wallet_delete_record_nonce' ) ) );
		}
	...
	function column_transaction_id( $item ) {
		echo '<strong>#' . esc_html( $item['transaction_id'] ) . '</strong>';
	}
	...
	public function prepare_items() {
		...
		$data = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}cubewp_dispute_requests", ARRAY_A );
	...
	public function get_columns() {
		return array(
			...
			'transaction_id' => esc_html__( 'Transaction #', 'cubewp-wallet' ),
